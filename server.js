...
// ─── FRONTEND ──────────────────────────────────────────────
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'nexus_frontend.html'));
});
...
app.get('/api/users', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, username, created_at, last_login FROM users ORDER BY created_at DESC LIMIT 100'
    );
    res.json(rows);
  } catch (e) {
    console.error(e);
...
app.put('/api/ventas/:id', auth, async (req, res) => {
  const { id } = req.params;
  const { producto, canal, monto, fecha } = req.body;
  if (!producto || !monto) return res.status(400).json({ error: 'producto y monto requeridos' });

  try {
    const { rows } = await pool.query(
      `UPDATE ventas SET producto=$1, canal=$2, monto=$3, fecha=COALESCE($4::date, fecha)
       WHERE id=$5 RETURNING *`,
      [producto, canal || 'Web', monto, fecha || null, id]
    );
...
app.delete('/api/ventas/:id', auth, async (req, res) => {
  const { id } = req.params;
  try {
    const { rowCount } = await pool.query('DELETE FROM ventas WHERE id=$1', [id]);
...
app.put('/api/users/:id', auth, async (req, res) => {
  const { id } = req.params;
  const { username, password } = req.body;
  if (!username && !password) return res.status(400).json({ error: 'username o password requeridos' });

  try {
    const values = [];
    const sets = [];
    if (username) { values.push(username.toLowerCase().trim()); sets.push(`username=$${values.length}`); }
    if (password) { const hash = await bcrypt.hash(password, 10); values.push(hash); sets.push(`password=$${values.length}`); }
    values.push(id);

    const { rows } = await pool.query(
      `UPDATE users SET ${sets.join(', ')} WHERE id=$${values.length} RETURNING id, username, created_at, last_login`,
      values
    );
